# -*- coding: utf-8 -*-
"""
Created on Fri Sep 22 19:11:49 2023
...
# 定义源文件
#src_pdb = "D:\\Research Project\\ll\\paper\\Codes\\pdb_files\\5ovf.pdb"
#dst_folder = './PDB_homo_processed_select_ligandsplit_4_5_3/'
#src_folder = './test_4/'
...
def compared_pocket_similarity(row,result_vector):
    
     # 计算相似度描述符之间的相似度
    data = np.array(list(map(float, row[11:373])))  # 提取第12列到第373列的数据
    similarity = np.dot(data, result_vector) / (np.linalg.norm(data) * np.linalg.norm(result_vector))  # 计算余弦相似性
    #计算曼哈顿距离
    #similarity = np.sum(np.abs(data - result_vector))
    return similarity
...
def process(src_pdb, num):
    # 文件夹路径
    folder_path = 'D:\\Research Project\\ll\\paper\\Codes/selected_base_pdb_pocket'
                    ...
                    if ligand.resname not in excluded_residues:
                        ...
                        if ligand.resname not in selected_ligands_name and len(ligand.resname)== 3:
                            ...
                            if binding_pocket_residues is not None:
                                ...
                                #计算本pdb与603个参考pdb的similarity列表
                                for file_name in file_list:
                                    file_path = os.path.join(folder_path, file_name)
                                    if os.path.isfile(file_path):  # 确保处理的是文件而非文件夹
                                        result = pocket_similarity(file_path,pdb_pocket_filepath)  # 调用处理函数处理文件
    ...
    ##########################################################################################################################
                             ...
                             #   df[['pocket_similarity']] = df.apply(compared_pocket_similarity, args=(result_vector,), axis=1, result_type='expand')
                                df['pocket_similarity'] = df.apply(compared_pocket_similarity, args=(result_vector,),
                                ...
                                # 提取前1000行
                                top_1000_rows = seq_similarity_df.head(num)

                                # 将前1000行转换为字典
                                #top_1000_dict = top_1000_rows.to_dict(orient='records')

                                # 选取第2、4列并另存为csv文件
                                top_1000_rows.to_csv('selected_pdbfile_by_compared_pocket_similarity_refined.csv', index=None)
                                ...
                                fragments_df.to_csv('fragments_file_by_compared_pocket_similarity_refined.csv', index=None)

                                # 将碎片库单独另存为csv文件
                                fragment_library = list(set(fragments_df['fragment'].tolist()))
                                fragment_library_df = pd.DataFrame(fragment_library, columns=['fragment'])
                                fragment_library_df.to_csv('fragment_library_file_by_compared_pocket_similarity_refined.csv', index=None)
                                ...
                                new_molecules_df.to_csv('new_molecules_file_by_compared_pocket_similarity_refined_filter.csv', index=None)
