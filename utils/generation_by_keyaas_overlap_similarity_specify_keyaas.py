...
def process(key_aas_list, num):
    ...
    # 选取第2、4列并另存为csv文件
    selected_df.to_csv('selected_pdbfile_by_keyaas_overlap_descending.csv', index=None)
    ...
    fragments_df.to_csv('fragments_file_by_keyaas_overlap_descending.csv', index=None)

    # 将碎片库单独另存为csv文件
    fragment_library = list(set(fragments_df['fragment'].tolist()))
    fragment_library_df = pd.DataFrame(fragment_library, columns=['fragment'])
    fragment_library_df.to_csv('fragment_library_file_by_keyaas_overlap_descending.csv', index=None)
