...
def process(src_pdb, num):
    ...
    ##########################################################################################################################
                              ...
                              #  df[['pocket_similarity']] = df.apply(pocket_similarity, args=(pdb_pocket_filepath,), axis=1, result_type='expand')
                                df['pocket_similarity'] = df.apply(pocket_similarity, args=(pdb_pocket_filepath,),
                                ...
                                # 选取第2、4列并另存为csv文件
                                top_1000_rows.to_csv('selected_pdbfile_by_pocket_similarity.csv', index=None)
                                ...
                                fragments_df.to_csv('fragments_file_by_pocket_similarity.csv', index=None)

                                # 将碎片库单独另存为csv文件
                                fragment_library = list(set(fragments_df['fragment'].tolist()))
                                fragment_library_df = pd.DataFrame(fragment_library, columns=['fragment'])
                                fragment_library_df.to_csv('fragment_library_file_by_pocket_similarity.csv', index=None)
