# -*- coding: utf-8 -*-
"""
Created on Wed Sep  6 16:01:21 2023
...
df[['pocket_seq_similarity', 'pocket_seq_similarity_start', 'pocket_seq_similarity_end']] = df.apply(calculate_similarity, args=(sequence,), axis=1, result_type='expand')
#计算口袋氨基酸序列的全局相似性
...
# 按照相似性降序排列
seq_similarity_df = df.sort_values(by='pocket_seq_similarity', ascending=False)
# 提取前1000行
...
# 选取第2、4列并另存为csv文件
top_1000_rows.to_csv('selected_pdbfile_by_pocket_seq_similarity_1000_2re9.csv', index=None)
...
fragments_df.to_csv('fragments_file_by_pocket_seq_similarity_2re9.csv', index=None)

# 将碎片库单独另存为csv文件
fragment_library = list(set(fragments_df['fragment'].tolist()))
fragment_library_df = pd.DataFrame(fragment_library, columns=['fragment'])
fragment_library_df.to_csv('fragment_library_file_by_pocket_seq_similarity_2re9.csv', index=None)
...
new_molecules_df.to_csv('new_molecules_file_by_pocket_seq_similarity_filter_10000_2re9.csv', index=None)
