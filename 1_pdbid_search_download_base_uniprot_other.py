...
def download_pdb(uniprot,pdb_ids):
    ...
    for pdbid in pdb_ids:
        ...
        # 指定PDB文件保存路径# 指定下载目录
        download_dir = f'./PDB_all/other/{uniprot}'
...
with open('./log_other_download.txt', 'w') as f:
    df = pd.read_csv("./chembl31_single_protein_list_deduplicated_other.csv", usecols=["UniProt_Accessions","sequence"])
    ...
    df.to_csv("./uniprot_pdb_list_other_protein.csv")
