# -*- coding: utf-8 -*-
"""
Created on Wed Sep  6 16:09:33 2023
...
def calculate_similarity(row, sequence):
    similarities = seq_similarity_local(str(row['pocket_sequence']), sequence)
...
def calculate_global_similarity(row, sequence):
    similarities = seq_similarity_global(str(row['pocket_sequence']), sequence)
    return pd.Series(similarities, index=['pocket_seq_similarity', 'pocket_seq_similarity_start', 'pocket_seq_similarity_end'])
...
# 将第10列（索引为9）字符串列表转换为真正的列表
df.iloc[:, 9] = df.iloc[:, 9].apply(ast.literal_eval)
# 计算重合度，并找出重合度大于0.75的行
selected_df = df[df.iloc[:, 9].apply(lambda x: overlap_ratio(x, key_aas_list)) > 0.75]
# 计算重叠比率并添加到 DataFrame 中
selected_df['overlap_ratio'] = selected_df['key_aas'].apply(lambda x: overlap_ratio(x, key_aas_list))

# 按照重叠比率降序排列
sorted_df = selected_df.sort_values(by='overlap_ratio', ascending=False)
selected_df = sorted_df.head(1000)
# 选取第2、4列并另存为csv文件
selected_df.to_csv('selected_pdbfile_by_keyaas_overlap_descending_nik.csv', index=None)

# 先计算第十列中包含'xx'字符的数量
df['aas_count'] = df.iloc[:, 9].apply(lambda x: x.count('TYR'))

# 按照'xx_count'列降序排列，并抽取前一千行
selected_df = df.sort_values(by='aas_count', ascending=False).head(1000)

# 移除额外添加的'xx_count'列
selected_df = selected_df.drop(columns=['aas_count'])
selected_df.to_csv('selected_pdbfile_by_keyaas_overlap_descending_tyr.csv', index=None)
'''


csv_file = 'selected_pdbfile_by_keyaas_overlap_descending_tyr.csv'
input_folder = 'PDB_processed_resolution_4_5_2'
output_folder = 'TYR'
...
with open(csv_file, 'r') as file:
    ...
    next(reader)  # 跳过标题行
    for row in reader:
        value = row[2].strip('"')  # 去除引号
        pdb_file = f"{value}.pdb"
        input_path = search_file(input_folder, pdb_file)
        output_path = os.path.join(output_folder, pdb_file)
        
        if input_path:
            # 复制文件到输出文件夹
            shutil.copy(input_path, output_path)
            print(f"已复制文件 {pdb_file} 到 {output_folder}")
        else:
            print(f"文件 {pdb_file} 不存在于 {input_folder} 或其子文件夹中")
'''
# 基于recap规则打碎分子
fragments = []
...
fragments_df.to_csv('fragments_file_by_keyaas_overlap_descending_nik.csv', index=None)

# 将碎片库单独另存为csv文件
fragment_library = list(set(fragments_df['fragment'].tolist()))
fragment_library_df = pd.DataFrame(fragment_library, columns=['fragment'])
fragment_library_df.to_csv('fragment_library_file_by_keyaas_overlap_descending_nik.csv', index=None)
...
new_molecules_df.to_csv('new_molecules_file_by_keyaas_overlap_filter_10000_nik.csv', index=None)
'''